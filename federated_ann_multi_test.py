...
class FederatedTrainer:
    def __init__(self, clients_data, input_dim, global_test_data):
        self.input_dim = input_dim
        self.clients_data = clients_data  # Format: {client: (X_train, y_train, X_test, y_test)}
        self.global_X_test, self.global_y_test = global_test_data
    ...
    def train_round(self, phase_number):
        ...
        for client, (X_train, y_train, _, _) in self.clients_data.items():
            try:
        ...
        if phase_number != 1:
            ...
            imp_weights = [self.round_wise_weights[client][-1] for client in imp_clients]
            aggregated_weights = self.federated_average(imp_weights)
        ...
        print(f"\nGlobal Metrics:")
        print(f"RMSE: {latest_entry['global_rmse']:.4f}, Accuracy: {latest_entry['global_accuracy']:.2f}%")
        print("\nClient Metrics:")
        for client in self.clients_data:
            metrics = latest_entry['clients_metrics'][client]
            print(f"{client}: RMSE={metrics['rmse']:.4f}, Accuracy={metrics['accuracy']:.2f}%")
    ...
    def evaluate_weights(self, weights, X_test, y_test):
        """Evaluate given weights on specified test data"""
        try:
        ...
        for (c1, c2) in combinations(active_clients, 2):
            ...
            for w1 in self.current_interval_weights[c1]:
                for w2 in self.current_interval_weights[c2]:
                    ...
                    print(f"client {c1} v\s client {c2} : similarity")
    ...
    def _record_history(self, round_counts):
        ...
        global_weights = self.aggregate_weights()
        entry['global_rmse'], entry['global_accuracy'] = self.evaluate_weights(
            global_weights, self.global_X_test, self.global_y_test
        )

        # Evaluate individual clients
        for client, (_, _, X_test, y_test) in self.clients_data.items():
            client_weights = self.client_weights[client]
            rmse, acc = self.evaluate_weights(client_weights, X_test, y_test)
            entry['clients_metrics'][client] = {'rmse': rmse, 'accuracy': acc}
            ...
            try:
                ...
                entry['diffs'][client] = np.linalg.norm(flat_client - flat_global)
            except Exception as e:
                print(f"Error calculating diffs for {client}: {str(e)}")
                entry['diffs'][client] = 0.0
...
def plot_training_progress(history):
    ...
    plt.plot(global_rmse, marker='o', label='Global')
    ...
    # Global Accuracy
    plt.subplot(1, 3, 2)
    global_acc = [entry['global_accuracy'] for entry in history]
    plt.plot(global_acc, marker='o', color='orange', label='Global')
    plt.title('Global Accuracy')
    plt.xlabel('Round')
    plt.grid(True)
    
    # Client Weight Differences
    plt.subplot(1, 3, 3)
...
def plot_client_metrics(history, clients):
    plt.figure(figsize=(12, 6))
    
    # Client RMSE
    plt.subplot(1, 2, 1)
    for client in clients:
        rmse_values = [entry['clients_metrics'][client]['rmse'] for entry in history]
        plt.plot(rmse_values, label=client)
    plt.title('Client RMSE Evolution')
    plt.xlabel('Round')
    plt.ylabel('RMSE')
    plt.legend()
    plt.grid(True)
    ...
    plt.subplot(1, 2, 2)
    for client in clients:
        acc_values = [entry['clients_metrics'][client]['accuracy'] for entry in history]
        plt.plot(acc_values, label=client)
    plt.title('Client Accuracy Evolution')
    plt.xlabel('Round')
    plt.ylabel('Accuracy (%)')
    plt.legend()
    plt.grid(True)
...
if __name__ == "__main__":
    ...
    # Load all clients with train/test data
    clients_data = {}
    for i in range(1, NUM_CLIENTS+1):
        # Load training data
        train_file = os.path.join(DATA_DIR, f'client_{i}.csv')
        X_train, y_train = load_client_data(train_file, global_scaler)
        
        # Load test data
        test_file = os.path.join(DATA_DIR, f'client_{i}_test.csv')
        X_test, y_test = load_client_data(test_file, global_scaler)
        
        clients_data[f'client_{i}'] = (X_train, y_train, X_test, y_test)
    
    # Prepare global test data
    test_df = pd.read_csv(TEST_FILE)
    X_test_global, y_test_global, _ = prepare_data(test_df, scaler=global_scaler, fit=False)
    
    # Initialize trainer
    trainer = FederatedTrainer(
        clients_data=clients_data,
        input_dim=X_test_global.shape[1],
        global_test_data=(X_test_global, y_test_global)
    )
    ...
    while len(trainer.history) < 100:
        if phase_number % RE_CLUSTER_INTERVAL == 0:
            ...
            plot_client_metrics(trainer.history, list(clients_data.keys()))
        ...
        if phase_number == 1:
            ...
            for round_num in range(INITIAL_TRAINING_ROUNDS):
                ...
                trainer.train_round(phase_number)
            ...
            for _ in range(LOCAL_TRAINING_INTERVAL):
                ...
                trainer._record_history({})  # Record metrics without new training
                latest_entry = trainer.history[-1]
                print(f"\nLocal Update Metrics:")
                print(f"Global RMSE: {latest_entry['global_rmse']:.4f}")
                print("Client Metrics:")
                for client in trainer.clients_data:
                    metrics = latest_entry['clients_metrics'][client]
                    print(f"{client}: RMSE={metrics['rmse']:.4f}")
